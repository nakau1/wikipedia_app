...
public class NBSlideMenuViewController: UIViewController, UIGestureRecognizerDelegate
{	
	...
	func initView()
	{
		...
		opacityView.backgroundColor = NBSlideMenuOptions.opacityViewBackgroundColor
		opacityView.autoresizingMask = [UIViewAutoresizing.FlexibleHeight, UIViewAutoresizing.FlexibleWidth]
		opacityView.layer.opacity = 0.0
		view.insertSubview(opacityView, atIndex: 1)
		
		var leftFrame: CGRect = view.bounds
		leftFrame.size.width = NBSlideMenuOptions.leftViewWidth
		...
		rightFrame.size.width = NBSlideMenuOptions.rightViewWidth
	...
	public func openLeftWithVelocity(velocity: CGFloat)
	{
		...
		var duration: NSTimeInterval = Double(NBSlideMenuOptions.animationDuration)
		if velocity != 0.0 {
		...
		UIView.animateWithDuration(duration, delay: 0.0, options: UIViewAnimationOptions.CurveEaseInOut, animations: { [weak self]() -> Void in
			if let strongSelf = self {
				...
				strongSelf.opacityView.layer.opacity = Float(NBSlideMenuOptions.contentViewOpacity)
				strongSelf.mainContainerView.transform = CGAffineTransformMakeScale(NBSlideMenuOptions.contentViewScale, NBSlideMenuOptions.contentViewScale)
			}
	...
	public func openRightWithVelocity(velocity: CGFloat)
	{
		...
		//	CGFloat finalXOrigin = NBSlideMenuOptions.rightViewOverlapWidth;
	...
	{
		return  -NBSlideMenuOptions.leftViewWidth
	}
	...
	private func panLeftResultInfoForVelocity(velocity: CGPoint) -> PanInfo
	{
		...
		let pointOfNoReturn: CGFloat = CGFloat(floor(leftMinOrigin())) + NBSlideMenuOptions.pointOfNoReturnWidth
	...
	{
		let thresholdVelocity: CGFloat = -1000.0
		let pointOfNoReturn: CGFloat = CGFloat(floor(CGRectGetWidth(view.bounds)) - NBSlideMenuOptions.pointOfNoReturnWidth)
	...
	private func applyLeftOpacity()
	{
		...
		let opacity: CGFloat = NBSlideMenuOptions.contentViewOpacity * openedLeftRatio
	...
	private func applyRightOpacity()
	{
		...
		let opacity: CGFloat = NBSlideMenuOptions.contentViewOpacity * openedRightRatio
		opacityView.layer.opacity = Float(opacity)
	}
	...
	{
		let openedLeftRatio: CGFloat = getOpenedLeftRatio()
		let scale: CGFloat = 1.0 - ((1.0 - NBSlideMenuOptions.contentViewScale) * openedLeftRatio);
	...
	{
		let openedRightRatio: CGFloat = getOpenedRightRatio()
		let scale: CGFloat = 1.0 - ((1.0 - NBSlideMenuOptions.contentViewScale) * openedRightRatio)
		mainContainerView.transform = CGAffineTransformMakeScale(scale, scale)
	}
	...
	private func addShadowToView(targetContainerView: UIView)
	{
		...
		targetContainerView.layer.shadowOffset = NBSlideMenuOptions.shadowOffset
		targetContainerView.layer.shadowOpacity = Float(NBSlideMenuOptions.shadowOpacity)
		targetContainerView.layer.shadowRadius = NBSlideMenuOptions.shadowRadius
	...
	private func addContentOpacity() {
		opacityView.layer.opacity = Float(NBSlideMenuOptions.contentViewOpacity)
	}
	...
	private func setOpenWindowLevel() {
		if (NBSlideMenuOptions.hideStatusBar) {
	...
	private func slideLeftForGestureRecognizer( gesture: UIGestureRecognizer, point:CGPoint) -> Bool{
		return isLeftOpen() || NBSlideMenuOptions.panFromBezel && isLeftPointContainedWithinBezelRect(point)
	}
	...
	private func isLeftPointContainedWithinBezelRect(point: CGPoint) -> Bool{
		...
		let bezelWidth: CGFloat = NBSlideMenuOptions.leftBezelWidth
	...
	private func slideRightViewForGestureRecognizer(gesture: UIGestureRecognizer, withTouchPoint point: CGPoint) -> Bool {
		return isRightOpen() || NBSlideMenuOptions.rightPanFromBezel && isRightPointContainedWithinBezelRect(point)
	}
	
	private func isRightPointContainedWithinBezelRect(point: CGPoint) -> Bool {
		var rightBezelRect: CGRect = CGRectZero
		var tempRect: CGRect = CGRectZero
		let bezelWidth: CGFloat = CGRectGetWidth(view.bounds) - NBSlideMenuOptions.rightBezelWidth
...
extension UIViewController {
	...
	public func slideMenuController() -> NBSlideMenuViewController? {
		var viewController: UIViewController? = self
		while viewController != nil {
			if viewController is NBSlideMenuViewController {
				return viewController as? NBSlideMenuViewController
			}
